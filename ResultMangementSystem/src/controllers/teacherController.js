...
export const createStudentRecord = async (req,res) => {
    ...
    if(req.body.score > 100 || req.body.score < 0){
        field.score =  "Score must be between 0-100";
        field.errors = true;
    }
    if(field.errors){
    ...
    res.render("teacher/studentCreate", { viewData: { student:unique , page: req.url, errors: { main: [ "Student record already exists!" ] } } });
}
...
export const createTeacherPost = async(req,res) => {
    ...
    if(req.body.firstName.length < 3){
        field.firstName =  "First Name Is Small";
        field.errors = true;
    }
    if(req.body.lastName.length < 3){
        field.lastName =  "Last Name Is Small";
        field.errors = true;
    }
    if(req.body.password.length < 8){
        field.password =  "Please Enter Strong PassWord";
        field.errors = true;
    }
